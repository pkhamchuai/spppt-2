...
# define model
class SP_Rigid(nn.Module):
    def __init__(self, model_params):
        super(SP_Rigid, self).__init__()
...
class AffineNet(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(self.conv4f*2, 2)
    ...
    def forward(self, source_image, target_image, points):
        ...
        x = self.aPooling(self.Act4(self.ReLU(self.conv4(x))))
        ...
        # print(identity.device, t.device)
        M = torch.cat((identity, t.view(2, 1)), dim=1)
        ...
        transformed_source_image_affine = tensor_affine_transform(source_image.detach().cpu(), M)
        ...
        transformed_points = transform_points_rigid(points[0].T.to(device), t)
