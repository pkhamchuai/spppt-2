...
def calculate_tre(source_landmarks, target_landmarks):
    tre = np.sqrt(np.square(source_landmarks[:, 0] - target_landmarks[:, 0]) + np.square(
        source_landmarks[:, 1] - target_landmarks[:, 1]))
    return tre
...
def compose_displacement_fields(u, v, device="cpu"):
    ...
    nv = torch.stack((u_x_2.view(u_x_2.size(0), u_x_2.size(2), u_x_2.size(3)),
                      u_y_2.view(u_y_2.size(0), u_y_2.size(2), u_y_2.size(3))), dim=3)
...
def fold(unfolded_tensor, padded_output_size, padding_tuple, patch_size, stride, device="cpu"):
    ...
    for j in range(col_y):
        for i in range(col_x):
            current_patch = unfolded_tensor[j * col_x + i, :, int(stride / 2):-int(stride / 2),
...
def build_pyramid(tensor, num_levels, device="cpu"):
    ...
    for i in range(num_levels):
        if i == num_levels - 1:
            ...
            new_size = torch.Size((current_size[0], current_size[1], int(current_size[2] / (2 ** (num_levels - i - 1))),
                                   int(current_size[3] / (2 ** (num_levels - i - 1)))))
