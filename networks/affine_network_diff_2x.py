...
class Affine_Network(nn.Module):
    ...
    def forward(self, source, target):
        x = self.feature_extractor(torch.cat((source, target, torch.abs(source-target)), dim=1))
        # x = x.view(1, -1)
        x = x.squeeze()
...
class Feature_Extractor(nn.Module):
    def __init__(self, device):
        ...
        self.input_layer = nn.Sequential(
            nn.Conv2d(3, 2*64, 7, stride=2, padding=3),
        )
        self.layer_1 = Forward_Layer(64*2, pool=True)
        self.layer_2 = Forward_Layer(128*2, pool=False)
        self.layer_3 = Forward_Layer(128*2, pool=True)
        self.layer_4 = Forward_Layer(256*2, pool=False)
        self.layer_5 = Forward_Layer(256*2, pool=True)
        self.layer_6 = Forward_Layer(512*2, pool=True)

        self.last_layer = nn.Sequential(
            nn.Conv2d(1024*2, 512, 3, stride=2, padding=1),
