...
def test(model_name, models, model_params, timestamp, verbose=False, plot=1, beam=1):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(testbar, 0):
            ...
            metrics_points_reverse = []
            metrics_images_forward = []
            metrics_images_reverse = []
            model_list = []
            no_improve = 0
            # accepted parameters
            ...
            '''
            To do the beam committee search, we need to do the following steps:
            Given b beam width, k models, and r repetitions:
            1. Forward direction: source -> target
            2. Reverse direction: target -> source
            3. Compute the metrics for each model, store the metrics in a list
            4. Choose the best b models based on the metrics
            5. Create b new source images using the best b models
            6. Repeat the steps 1-5 for r times, in the iteration > 1, use the best b models from the previous iteration            
            7. Stop the iteration if there is no improvement for 2 consecutive iterations          
            8. Save the last metrics to a csv file
            '''
            ...
            for j in range(rep):
                for b in range(beam):
                    ...
                    for b in range(beam):
                        model_number = best_index[b]//2
                        if verbose:
                            print(f"Pair {i}, Rep {j}, best model index: {best_index}")

                        if best_index % 2 == 0:
                            outputs = model[model_number](source_image, target_image, points1)
                            transformed_source_affine = outputs[0]
                            affine_params_predicted = outputs[1]
                            points1_2_predicted = outputs[2]
                            ...
                            outputs = model[model_number](target_image, source_image, points2)
                            # transformed_source_affine = outputs[0]
                            affine_params_predicted_rv = outputs[1]
                        ...
                        if i < 100 and plot == 1:
                            plot_ = 1
                            best_model_text = f"rep{j:02d}_{best_index//2}_{b}_fw" if best_index % 2 == 0 else f"rep{j:02d}_{best_index//2}_{b}_rv"
                        ...
                        if min_metrics_points < metrics_points_best or min_mse_images < mse_images_best: # TODO: add SSIM    
                            ...
                            source_beam[b] = tensor_affine_transform0(data[0].to(device), M)
